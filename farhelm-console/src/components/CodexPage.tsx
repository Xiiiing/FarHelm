...
export function CodexPage({ csrf, agents }: { csrf: string; agents: AgentSummary[] }) {
  ...
  const rail = <aside className="codex-rail" aria-label="项目和会话">
    <div className="codex-rail-head"><h2>会话</h2><div className="rail-heading-actions">{projects.length > 0 && <ScopeFilter value={scope} options={scopes} onChange={setScope} />}<Button type="text" className="new-session-button" icon={<PlusOutlined />} onClick={newSession} aria-label="新建会话">新建</Button></div></div>
    ...
    <div className="session-filter-row"><Segmented block className="archive-tabs" value={archive} onChange={(value) => { setArchive(value as ArchiveFilter); select(undefined) }} options={[{ label: '当前', value: 'false' }, { label: '归档', value: 'true' }, { label: '全部', value: 'all' }]} /></div>
  ...
  return <div className={`codex-workspace ${collapsed ? 'rail-collapsed' : ''} ${entering ? 'workspace-entering' : ''}`}><div className="codex-desktop-rail">{rail}</div><Drawer className="codex-mobile-drawer" aria-label="项目和会话" placement="left" open={railOpen} onClose={() => setRailOpen(false)} afterOpenChange={(open) => { if (!open && createAfterRail) { setCreateAfterRail(false); setCreating(true) } }} size={Math.min(320, window.innerWidth - 24)} closable={false}>{rail}</Drawer>
