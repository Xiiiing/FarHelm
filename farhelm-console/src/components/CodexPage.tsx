...
import { Alert, Button, Card, Empty, Form, Input, List, Modal, Radio, Select, Space, Tag, Typography } from 'antd'
...
export function CodexPage({ csrf }: { csrf: string }) {
  const [searchParams] = useSearchParams(); const targetSession = searchParams.get('session'); const [sessions, setSessions] = useState<CodexSession[]>([]); const [selected, setSelected] = useState<CodexSession>(); const [error, setError] = useState<string>(); const [creating, setCreating] = useState(false); const [replies, setReplies] = useState<Record<string, string>>({}); const [projectQuery, setProjectQuery] = useState(''); const [projectFilter, setProjectFilter] = useState(''); const [form] = Form.useForm()
  const load = useCallback(() => { void fetchSessions(projectFilter || undefined).then((items) => { setSessions(items); setSelected((current) => items.find((item) => item.session_id === (current?.session_id ?? targetSession)) ?? current) }).catch((reason: unknown) => setError(reason instanceof Error ? reason.message : 'Codex 会话不可用')) }, [projectFilter, targetSession])
  useEffect(() => {
  ...
  return <section className="feature-page" aria-labelledby="codex-title"><div className="page-heading"><div><Typography.Text className="eyebrow">CODEX THREADS</Typography.Text><Typography.Title id="codex-title" level={1}>Codex</Typography.Title><Typography.Paragraph type="secondary">恢复旧会话，或在只读项目/隔离 worktree 中创建新会话。</Typography.Paragraph></div><Space wrap><Input.Search aria-label="按项目筛选会话" placeholder="项目 ID" value={projectQuery} onChange={(event) => setProjectQuery(event.target.value)} onSearch={(value) => setProjectFilter(value.trim())} /><Button icon={<ReloadOutlined />} onClick={load}>刷新</Button><Button type="primary" icon={<PlusOutlined />} onClick={() => setCreating(true)}>新会话</Button></Space></div>
