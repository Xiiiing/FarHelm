import { CheckCircleOutlined, DisconnectOutlined, PlusOutlined, ReloadOutlined } from '@ant-design/icons'
...
export function AgentListPage({ csrf, agents, onRefresh }: { csrf: string; agents: AgentsState; onRefresh: () => void }) {
  ...
  const { catalog: projectsQuery, projects } = useProjects(csrf)
  const [projectsOpen, setProjectsOpen] = useState(false)
  const [creatingProject, setCreatingProject] = useState<ProjectCandidate>()
  ...
  return <section aria-labelledby="agents-title" className="feature-page">
    ...
    <div className="page-heading section-heading"><div><Typography.Text className="eyebrow">DISCOVERED PROJECTS</Typography.Text><Typography.Title level={2}>Codex 项目</Typography.Title><Typography.Paragraph type="secondary">从已发现项目导入、接入已有目录，或在授权根目录下创建空项目。</Typography.Paragraph></div><Button type="primary" icon={<PlusOutlined />} onClick={() => setProjectsOpen(true)}>添加与管理项目</Button></div>
    {projectsQuery.error && <Alert showIcon type="warning" title="项目读取失败" description={projectsQuery.error.message} action={<Button onClick={loadProjects}>重试</Button>} />}
    <Card className="agent-list-card" styles={{ body: { padding: projects.length ? 0 : 24 } }}>{projects.length === 0 ? <Empty description="尚无项目，点击“添加与管理项目”接入或创建。" /> : <List dataSource={projects} renderItem={(project) => <List.Item className="agent-row"><List.Item.Meta title={<Space wrap><span>{project.display_name}</span><Tag color={project.state === 'approved' ? 'success' : 'processing'}>{project.state === 'approved' ? '已导入' : '待确认'}</Tag></Space>} description={`${project.agent_id} · ${project.suggested_project_id}`} /><Typography.Text type="secondary">{project.session_count} 个会话</Typography.Text></List.Item>} />}</Card>

    <ProjectsDrawer csrf={csrf} agents={agents.state === 'ready' ? agents.data.agents : []} open={projectsOpen} onClose={() => setProjectsOpen(false)} onNew={setCreatingProject} initialTab="add" />
    {creatingProject && <CreateDialog csrf={csrf} projects={projects} initialProject={creatingProject} onClose={() => setCreatingProject(undefined)} onCreated={(id) => { if (id) void navigate(`/codex?session=${encodeURIComponent(id)}`) }} />}
