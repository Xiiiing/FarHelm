import { CheckCircleOutlined, CloseCircleOutlined, CopyOutlined, LoadingOutlined, ToolOutlined } from '@ant-design/icons'
...
const Message = memo(function Message({ item }: { item: TranscriptItem }) {
  ...
  return <article className={`codex-message ${role}`}><div className="message-role">{role === 'user' ? '你' : role === 'error' ? '执行错误' : 'Codex'}{item.streaming && <span className="stream-caret" aria-label="正在回复" />}</div><div className="message-body">{role === 'user' ? <div className="user-text">{item.text}</div> : <Markdown text={item.text} />}</div>{role === 'assistant' && !item.streaming && <Actions className="message-actions" fadeIn={false} items={[{ key: 'copy', actionRender: <Button type="text" aria-label={copied ? '已复制回复' : '复制回复'} icon={copied ? <CheckCircleOutlined /> : <CopyOutlined />} onClick={() => { void navigator.clipboard.writeText(item.text).then(() => setCopied(true), () => setCopied(false)) }} /> }]} />}</article>
})
...
const Turn = memo(function Turn({ turn, latest, continuation, loading, onContinue, onAnchor }: Omit<Props, 'turns' | 'scroll' | 'position'> & { turn: TranscriptTurn; latest: boolean }) {
  ...
  return <section className="codex-turn" data-turn-id={turn.turn_id} aria-label={`对话 ${turn.turn_id.slice(0, 8)}`}><header className="turn-heading"><h2>{latest ? '最新对话' : '历史对话'}</h2>{turn.started_at_unix && <time>{new Date(turn.started_at_unix * 1000).toLocaleString()}</time>}<span className="turn-state">{stateNames[turn.status] ?? '状态未知'}</span></header>{turn.items.map((item) => isTool(item) && item.item_id !== firstTool ? null : <div className="transcript-part" key={item.item_id} data-message-key={`${turn.turn_id}:${item.item_id}`}>
