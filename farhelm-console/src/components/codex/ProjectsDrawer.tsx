...
export function ProjectsDrawer({ csrf, agents, open, onClose, onNew, initialTab = 'manage' }: { csrf: string; agents: AgentSummary[]; open: boolean; onClose: () => void; onNew?: (project: ProjectCandidate) => void; initialTab?: string }) {
  ...
  const batchImport = (items: ProjectCandidate[]) => operation.mutate(async () => {
    ...
    for (const project of items) {
      try { await waitForCommand(await importProjects(csrf, project.agent_id, [project.candidate_id])); succeeded++ }
      catch (error) { failures.push(`${project.display_name}：${errorText(error)}`) }
      await model.catalog.refetch()
    }
    setSelected([])
    setSuccess(failures.length ? `已接入 ${succeeded} 个；${failures.length} 个失败。${failures.join('；')}` : '项目已接入，历史会话正在同步。')
  })
  ...
  const manage = <Space orientation="vertical" className="project-manager-stack" size="middle">
    ...
    <Space wrap><Button disabled={busy || !model.preferences.data || !model.approved.length} onClick={() => model.save.mutate(model.approved.map((p) => ({ ...model.preference(p), hidden: false })))}>全选展示</Button><Button disabled={busy || !model.preferences.data || !model.approved.length} onClick={() => model.save.mutate(model.approved.map((p) => ({ ...model.preference(p), hidden: true })))}>取消全选</Button><Button disabled={busy || !model.approved.some((p) => model.preference(p).manual_order !== undefined)} onClick={() => model.save.mutate(model.approved.map((p) => ({ ...model.preference(p), manual_order: undefined })))}>恢复默认排序</Button><Button icon={<PlusOutlined />} onClick={() => setTab('add')}>添加项目</Button></Space>
    <List loading={model.catalog.isPending} locale={{ emptyText: '尚未接入项目，请添加项目' }} dataSource={model.approved} rowKey={projectKey} renderItem={(p) => {
      ...
      return <List.Item className="project-manager-row" draggable={!busy} onDragStart={() => setDragging(key)} onDragEnd={() => setDragging(undefined)} onDragOver={(event) => event.preventDefault()} onDrop={() => { if (!dragging || dragging === key) return; const ordered = [...model.approved]; const from = ordered.findIndex((item) => projectKey(item) === dragging); const to = ordered.findIndex((item) => projectKey(item) === key); if (from < 0 || to < 0) return; const [moved] = ordered.splice(from, 1); ordered.splice(to, 0, moved); setDragging(undefined); model.save.mutate(ordered.map((item, index) => ({ ...model.preference(item), manual_order: index }))) }}><div className="project-manager-title"><Checkbox checked={!pref.hidden} disabled={busy || !model.preferences.data} onChange={(e) => model.save.mutate([{ ...pref, hidden: !e.target.checked }])} aria-label={`展示 ${p.display_name} · ${p.agent_id}`} /><div><strong>{p.display_name}</strong><div className="settings-detail-note">拖动排序 · {p.agent_id} · {p.suggested_project_id}</div></div>{pref.pinned && <PushpinOutlined aria-label="已置顶" />}</div><Space wrap>
    ...
    {canBrowse && <>
      ...
      {!!roots.data?.roots.length && <><Select aria-label="授权根目录" placeholder="选择授权根目录" value={directory?.startsWith('rtd_') ? directory : undefined} loading={roots.isFetching} options={roots.data.roots.map((r) => ({ value: r.directory_id, label: r.name }))} onChange={(v) => { setDirectory(v); operation.reset() }} />
        {page && <><Space wrap>{page.parent_id && <Button icon={<ArrowLeftOutlined />} onClick={() => setDirectory(page.parent_id!)}>上一级</Button>}<Typography.Text strong>当前目录：{page.directory.name}</Typography.Text></Space>
          ...
          {mode === 'link' && <Select aria-label="关联到项目" placeholder="选择同一服务器上的项目" value={linkProject} onChange={setLinkProject} options={model.approved.filter((p) => p.agent_id === agent?.agent_id).map((p) => ({ value: p.suggested_project_id, label: p.display_name }))} />}
          <Button type="primary" loading={operation.isPending} disabled={busy || !!directories.error || (mode === 'attach' ? directory?.startsWith('rtd_') : mode === 'link' ? directory?.startsWith('rtd_') || !linkProject : !name || name.trim() !== name || (/[/\\]/.test(name) || [...name].some((c) => c.charCodeAt(0) < 32 || c.charCodeAt(0) === 127)) || ['.', '..'].includes(name) || name.startsWith('.farhelm-'))} onClick={() => operation.mutate(async () => {
            await waitForCommand(await addProject(csrf, agent!.agent_id, directory!, mode === 'create' ? name : undefined, mode === 'link' ? linkProject : undefined))
            setName(''); setSuccess('项目已接入，可在“展示管理”中新建会话。历史同步会在后台继续。'); setTab('manage')
          })}>{mode === 'create' ? '创建并接入空项目' : mode === 'link' ? '关联为项目工作目录' : '接入当前目录'}</Button>
  ...
  return <><Drawer className="project-management-drawer" title="项目管理" aria-label="项目管理" open={open} onClose={onClose} size={Math.min(560, window.innerWidth)} afterOpenChange={(visible) => { if (!visible && nextSession) { onNew?.(nextSession); setNextSession(undefined) } else if (!visible && window.innerWidth < 768) { document.querySelector<HTMLButtonElement>('button[aria-label="打开会话列表"]')?.focus() } }}>
    ...
    <Tabs activeKey={tab} onChange={setTab} items={[{ key: 'manage', label: '展示管理', children: manage }, { key: 'add', label: '添加项目', children: <Space orientation="vertical" className="project-manager-stack" size="middle"><Segmented block value={mode} onChange={(v) => { setMode(v); operation.reset() }} options={[{ value: 'discovered', label: '导入已发现项目' }, { value: 'attach', label: '接入已有目录' }, { value: 'create', label: '创建空项目' }, { value: 'link', label: '关联更多目录' }]} />{mode === 'discovered' ? discovered : browse}</Space> }]} />
