...
export function Conversation({ recent, onSelect, onNew, onBrowse, csrf, id, session: listed, draft, onDraft, onSend, onRail, onCollapse, collapsed, onSchedule, onSchedules }: Props) {
  ...
  const submit = () => { onSend(visibleTurn); sender.current?.focus({ preventScroll: true }) }
  return <section className={`codex-conversation ${!id ? 'conversation-unselected' : ''}`}>
    <header className="conversation-head">
      <div className="conversation-identity">
        ...
        <div className="conversation-title"><div className="conversation-meta">{session ? <><FolderOpenOutlined /><span>{session.project_id}</span><span className="context-separator">/</span><span>{session.agent_id}</span></> : '你的项目，你的工作进度'}</div><h1>{session ? sessionName(session) : id ? '正在读取会话…' : 'Codex 工作区'}</h1></div>
      </div>
    ...
    {id && <footer className="composer-wrap">
      <Sender ref={sender} className={`composer ${draft.text.trim() ? 'has-draft' : ''} ${draft.sending ? 'is-submitting' : ''}`} components={senderComponents} value={draft.text} onChange={(value) => onDraft((old) => ({ ...old, text: value }))} autoSize={{ minRows: 1, maxRows: 5 }} placeholder={session ? '描述你的下一步，或提出一个问题…' : '正在读取会话…'} disabled={!session || failureCode === 'session_not_found'} onSubmit={submit} onKeyDown={(event) => { if (event.nativeEvent.isComposing || event.keyCode === 229) return false }} suffix={false} footer={(_, { components: { SendButton } }) => <>
        ...
        <div className="composer-actions">
          <Space size={4}><Tooltip title={session?.mode === 'edit' ? '允许在隔离工作区中编辑' : '只读模式：分析与检查，不修改项目'}><span className="composer-context">{session?.mode === 'edit' ? <EditOutlined /> : <EyeOutlined />}<span>{session?.mode === 'edit' ? '编辑' : '只读'}</span></span></Tooltip><span className="composer-action-divider" /><Tooltip title="在指定时间或训练成功后发送"><Button type="text" className="schedule-action" icon={<ClockCircleOutlined />} disabled={!session} onClick={() => session && onSchedule(session)} aria-label="定时发送"><span>定时发送</span></Button></Tooltip>{visibleTurn && <Button danger type="text" icon={<StopOutlined />} onClick={interrupt} aria-label="中断">中断</Button>}</Space>
          <SendButton className="send-action" shape="default" type="primary" icon={<ArrowUpOutlined />} loading={draft.sending} disabled={!session || !draft.text.trim() || draft.sending} aria-label="发送指令"><span className="send-action-label">发送</span></SendButton>
        </div>
      </>} />
