...
外观以黑白灰为主，设置中可独立选择浅深模式及强调色（石墨、蓝、绿、紫、玫瑰、橙或自定义颜色），即时生效并保存在当前浏览器。圆润的输入框固定可见；界面使用本地加载的 Manrope 与 Noto Sans SC 字体和统一阅读列。提交与展开过程提供简短动效，遵循系统“减少动态效果”设置；已缓存会话切换不重播进入动画。
